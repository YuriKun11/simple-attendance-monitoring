...
if (isset($_POST['update_info'])) {
    ...
    if (!$full_name || !$email) {
        ...
        $stmt = $conn->prepare("UPDATE drivers SET full_name=?, email=? WHERE driver_id=?");
        $stmt->bind_param("ssi", $full_name_clean, $email_clean, $driver_id);
        
        if ($stmt->execute()) {
            $message = 'Profile updated successfully!';
            $_SESSION['driver_name'] = $full_name_clean; 
        } else {
...
$stmt = $conn->prepare("SELECT full_name, email FROM drivers WHERE driver_id=?");
...
<div class="w-full flex justify-center bg-gray-50 py-8 sm:py-10">
    <div class="w-full lg:max-w-2xl bg-white rounded-xl shadow-2xl border border-gray-200 p-6 sm:p-8">
        <h2 class="text-2xl font-bold text-gray-900 mb-6 border-b pb-3 flex items-center">
             <i class="fas fa-car-side text-blue-600 mr-2"></i> Driver Account Settings
        </h2>
